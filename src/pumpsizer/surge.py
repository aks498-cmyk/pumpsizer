...
def wave_celerity(
    ...
    *,
    ...
    denom = (
        1.0 + (bulk_modulus_pa / youngs_modulus_pa) * (diameter_m / wall_thickness_m) * restraint
    )
...
def joukowsky_head(celerity_m_s: float, delta_velocity_m_s: float, g: float = G) -> float:
    """Head rise (or drop) for a rapid velocity change |dv|:  dh = a*dv/g  [m]."""
    return celerity_m_s * abs(delta_velocity_m_s) / g


def slow_closure_head(
    length_m: float, velocity_m_s: float, closure_time_s: float, g: float = G
) -> float:
...
def surge_head(
    ...
    *,
    ...
    return (
        (michaud, f"Michaud slow-closure (T_close={closure_time_s:.1f}s > Tc={tc:.2f}s)")
        if michaud < jouk
        else (jouk, "Joukowsky (Michaud bound not lower)")
    )
...
# ---------------------------------------------------------------------------
def air_vessel_prelim(
    *,
    pipe_area_m2: float,
    length_m: float,
    velocity_m_s: float,
    static_head_m: float,
    allowable_max_head_m: float,
    allowable_min_head_m: float,
    rho: float = 1000.0,
    g: float = G,
    gas_law_n: float = 1.2,
    normal_gas_fraction: float = 0.5,
) -> dict:
...
def flywheel_prelim(
    *,
    ...
    speed_rpm: float,
    target_stop_time_s: float,
    pump_motor_inertia_kgm2: float | None = None,
    radius_of_gyration_m: float = 0.3,
...
def assess(
    *,
    ...
    speed_rpm: float = 1480.0,
    allowable_max_head_m: float | None = None,
    allowable_min_head_m: float = 0.0,
) -> SurgeAssessment:
    """Full rule-of-thumb assessment for one pumping main + protection pre-size."""
    a = wave_celerity(diameter_m, wall_thickness_m, youngs_modulus_pa, rho=rho, restraint=restraint)
    tc = pipe_period(length_m, a)
    dh, rule = surge_head(length_m, steady_velocity_m_s, a, closure_time_s=closure_time_s)
    ...
    if sep_risk:
        recs.append(
            "down-surge approaches vapour pressure - column separation "
            "likely; provide an air vessel, one-way surge tank or air valves"
        )
    if exceeds:
        recs.append(
            f"up-surge {h_max:.1f} m exceeds pipe rating "
            f"{pipe_rating_head_m:.1f} m - protection or a higher class needed"
        )
    if not needs:
        recs.append(
            "rule-of-thumb surge within limits; a transient check is still "
    ...
    if needs and allowable_max_head_m is not None:
        av = air_vessel_prelim(
            pipe_area_m2=area,
            length_m=length_m,
            velocity_m_s=steady_velocity_m_s,
            static_head_m=static_head_m,
            allowable_max_head_m=allowable_max_head_m,
            allowable_min_head_m=max(allowable_min_head_m, 2.0),
            rho=rho,
        )
    fw = None
    if needs and shaft_power_kw:
        fw = flywheel_prelim(
            shaft_power_kw=shaft_power_kw, speed_rpm=speed_rpm, target_stop_time_s=max(tc, 1.0)
        )
    ...
    return SurgeAssessment(
        ...
        celerity_m_s=a,
        pipe_period_s=tc,
        steady_velocity_m_s=steady_velocity_m_s,
        static_head_m=static_head_m,
        surge_head_m=dh,
        surge_rule=rule,
        max_head_m=h_max,
        min_head_m=h_min,
        column_separation_risk=sep_risk,
        exceeds_rating=exceeds,
        pipe_rating_head_m=pipe_rating_head_m,
        protection_needed=needs,
