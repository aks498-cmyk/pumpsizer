...
class ProjectResults:
    ...
    def summary(self) -> dict:
        return {
            ...
            "operating_points_extra": {
                k: v.as_dict() for k, v in self.operating_points_extra.items()
            },
...
class Project:
    ...
    # ------------------------------------------------------------------
    def run(
        self, pipe_db: PipeDatabase | None = None, fittings: FittingCatalog | None = None
    ) -> ProjectResults:
        d = self.data
        warnings: list[str] = []
        ...
        for spec in seg_specs:
            ...
            if spec.get("diameter_mm"):
                ...
                warnings.append(
                    f"segment {name!r}: auto-sized to DN{int(choice.dn)} "
                    f"(ID {idmm:.1f} mm, v={choice.velocity_at_q:.2f} m/s)"
                )
            else:
                raise ValueError(
                    f"segment {name!r} needs 'dn' or 'diameter_mm' (or set autosize_diameter: true)"
                )
            diameters[name] = idmm
        ...
        if {"reservoir_hwl_m", "reservoir_bwl_m", "sump_hwl_m", "sump_bwl_m"} <= set(lv):
            ...
            h_static_max = float(
                _get(d, "suction.static_head_max_m", _get(d, "suction.static_head_m", 0.0))
            )
        ...
        def make_curve(h_static: float, roughness_mm: float, label: str) -> SystemCurve:
            segs = [
                PipeSegment(s.name, s.length_m, s.diameter_mm, roughness_mm, s.hazen_williams_c)
                for s in all_segments
            ]
            return SystemCurve(
                static_head=h_static,
                segments=segs,
                minor_losses=all_minor,
                kinematic_viscosity=water.kinematic_viscosity,
                method=method,
                roughness_condition=label.split("/")[-1],
                label=label,
            )
        ...
        # -- duty point per pump -------------------------------
        duty_q_pp = (
            float(_get(d, "pump.duty.flow_lps", q_total / max(n_duty, 1) / LPS_TO_M3S)) * LPS_TO_M3S
            if _get(d, "pump.duty.flow_lps")
            else q_total / max(n_duty, 1)
        )
        ...
        try:
            extra["min_static_new"] = (
                solve_operating_point(pump, system_set.min_static_new, rho=rho, g=G)
                if arrangement != "parallel" or n_run == 1
                else solve_parallel(pump, system_set.min_static_new, n_run, rho=rho, g=G)
            )
        except ValueError as exc:  # pragma: no cover - depends on data
            warnings.append(f"min-static operating point: {exc}")

        # -- NPSH -----------------------------------------
        npsh = self._npsh_check(
            d, water, system_set, minor_by_group, seg_by_group, diameters, op, warnings
        )
        ...
        if hours > 0:
            ...
            if _get(d, "energy.life_cycle_years"):
                energy["life_cycle_cost"] = life_cycle_cost(
                    float(_get(d, "energy.capital_cost", 0.0)),
                    cost,
        ...
        export = build_pump_export(
            pump,
            pump_id=ep.get("pump_id", "PMP1"),
            from_node=ep.get("from_node", "SUC"),
            to_node=ep.get("to_node", "DIS"),
        ...
        return ProjectResults(
            ...
            water=water,
            diameters_mm={k: round(v, 2) for k, v in diameters.items()},
            system_set=system_set,
            design_system_head_m=design_head,
            duty_flow_per_pump_m3s=duty_q_pp,
            duty_head_m=duty_h,
            pump=pump,
            operating_point=op,
            operating_points_extra=extra,
            npsh=npsh,
            motor=motor,
            energy=energy,
            epanet_export=export,
            selection=selection,
            surge=surge,
            staging=staging,
            warnings=warnings,
    ...
    @staticmethod
    def _run_staging(
        d, stg, pump, design_system, system_set, q_total, n_duty, lv, tariff, water, warnings
    ):
        from .staging import DemandPattern, StagingConfig, Tank, simulate_staging

        n_avail = int(stg.get("n_pumps_available", n_duty + int(_get(d, "flow.standby_pumps", 1))))
        ...
        tank = Tank(
            ...
            start_level_m=tcfg.get("start_level_m"),
            stop_level_m=tcfg.get("stop_level_m"),
        ...
        cfg = StagingConfig(
            n_pumps_available=n_avail,
            mode=str(stg.get("mode", "vfd")).lower(),
        ...
        return simulate_staging(
            ...
            cfg,
            rho=water.density,
            days=int(stg.get("days", 1)),
            tariff_per_kwh=tariff,
            motor_poles=int(_get(d, "motor.poles", 2)),
    ...
    def _surge_assessment(
        ...
        result = _surge.assess(
            ...
            static_head_m=float(h_static_max),
            rho=rho,
            closure_time_s=wh.get("closure_time_s"),
            pipe_rating_head_m=pn,
            restraint=float(wh.get("restraint", 1.0)),
            shaft_power_kw=motor.shaft_power_kw,
            speed_rpm=rpm,
        ...
        if str(wh.get("method", "rule_of_thumb")).lower() == "moc":
            ...
            if inertia is None:
                ...
                warnings.append(
                    f"water_hammer.moc: pump+motor inertia not given; estimated {inertia:.1f} kg.m2"
                )
            pipe_obj = _tr.Pipeline.from_pipe(
                length_m=length,
                diameter_mm=rm.diameter_mm,
                wall_thickness_mm=e_mm,
                youngs_modulus_pa=E_pa,
                friction_factor=0.017,
                pump_elevation_m=pump_el,
                reservoir_elevation_m=res_el,
                reaches=int(wh.get("moc_reaches", 24)),
                rho=rho,
            )
            pump_obj = _tr.PumpInertia(
                rated_speed_rpm=rpm,
                rated_flow_m3s=op.flow_per_pump_m3s,
                rated_head_m=op.head_m,
                total_inertia_kgm2=float(inertia),
                rated_efficiency=max((op.efficiency_pct or 80.0) / 100.0, 0.4),
            )
            av = None
            if wh.get("air_vessel_gas_volume_m3"):
                av = _tr.AirVessel(
                    gas_volume_m3=float(wh["air_vessel_gas_volume_m3"]),
                    polytropic_n=float(wh.get("polytropic_n", 1.2)),
                )
            tr = _tr.simulate_pump_trip(
                pipe_obj,
                pump_obj,
                sump_level_m=pump_el,
                reservoir_level_m=res_el,
                rho=rho,
                air_vessel=av,
    ...
    def _select_from_catalogue(d, duty_q, duty_h, design_system, warnings):
        ...
        if not feasible:
            raise ValueError(
                f"no catalogue pump can meet the duty {duty_q * 1000:.0f} l/s @ {duty_h:.1f} m"
            )
        pick_name = _get(d, "pump.model")
        chosen = next(
            (c for c in feasible if pick_name and pick_name.lower() in c.model.key.lower()),
            feasible[0],
        )
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def _build_pump_curve(d: dict, duty_q: float, duty_h: float, warnings: list[str]) -> PumpCurve:
        src = str(_get(d, "pump.source", "synthetic")).lower()
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def _npsh_check(d, water, system_set, minor_by_group, seg_by_group, diameters, op, warnings):
        lv = _get(d, "levels", {}) or {}
        ...
        if static_suction is None:
            if "sump_bwl_m" in lv and centreline is not None:
                ...
                static_suction = 0.0
                warnings.append(
                    "levels.pump_centreline_m not given; assuming pump at "
        ...
        suc = SystemCurve(
            static_head=0.0,
            segments=seg_by_group.get("suction", []),
            minor_losses=minor_by_group.get("suction", []),
            kinematic_viscosity=water.kinematic_viscosity,
            method=system_set.design().method,
            label="suction",
        )
        ...
        ref_d = diameters.get(fit_ref) or (
            seg_by_group["suction"][-1].diameter_mm if seg_by_group.get("suction") else None
        )
        v_head = 0.0
        if ref_d:
        ...
        return npsh_available(
            atmospheric_head_m=float(
                _get(d, "suction.atmospheric_head_m", water.atmospheric_pressure_head)
            ),
            ...
            vapour_pressure_head_m=(
                0.0
                if _get(d, "suction.ignore_vapour_pressure", False)
