...
class OperatingPoint:
    ...
    def as_dict(self) -> dict:
        return {
            ...
            else round(self.efficiency_pct, 2),
...
def _intersect(pump: PumpCurve, system: SystemCurve, q_hi: float) -> float:
    ...
    if d_lo < 0:
        ...
        raise ValueError(
            "pump shut-off head is below the system static head; no operating point exists"
        )
    if d_lo * d_hi > 0:
        # scan for a bracket
...
def _make_point(
    ...
    return OperatingPoint(
        ...
        pump_label=pump.name,
        note=note,
    )


def solve_operating_point(
    pump: PumpCurve, system: SystemCurve, *, rho: float = 1000.0, g: float = 9.81
) -> OperatingPoint:
    """Single-pump operating point."""
    q = _intersect(pump, system, pump.max_flow())
    return _make_point(pump, system, q, n_pumps=1, speed_ratio=pump.speed_ratio, rho=rho, g=g)


def solve_parallel(
    pump: PumpCurve, system: SystemCurve, n_pumps: int, *, rho: float = 1000.0, g: float = 9.81
) -> OperatingPoint:
...
def solve_vfd_speed(
    ...
    *speed-scaled* pump curve with the *unscaled* system curve.
    ...
    combined = trial if n_pumps == 1 else PumpCurve.parallel([trial] * n_pumps)
    q = _intersect(combined, system, combined.max_flow())
    return _make_point(trial, system, q, n_pumps=n_pumps, speed_ratio=s, rho=rho, g=g, note=note)
