...
def text_report(res: ProjectResults) -> str:
    ...
    for label, sc in res.system_set.as_dict().items():
        L.append(
            _row(f"H_sys @ {q_design * 1000:.0f} l/s  [{label}]", f"{sc.head(q_design):.2f}", "m")
        )
    L.append(_row("Design system head (max static/used)", f"{res.design_system_head_m:.2f}", "m"))
    L.append(
        _row(
            "Design duty (per pump)",
    ...
    if res.selection:
        ...
        for i, c in enumerate(res.selection[:8], 1):
            ...
            L.append(
                f"  {i:>2}  {c.model.key:<26}{c.method:<9}{ef:>6}"
    ...
    for key, ep in res.operating_points_extra.items():
        L.append(_row(f"[{key}] flow / head", f"{ep.flow_lps:.1f} l/s @ {ep.head_m:.2f} m", ""))
    ...
    if res.surge is not None:
        ...
        L.append(
            _row(
                "Rising main assessed", f"{s.length_m:.0f} m x {s.diameter_m * 1000:.0f} mm ID", ""
            )
        )
        ...
        if s.flywheel:
            ...
            L.append(
                _row(
                    f"  flywheel mass @ k={fw['radius_of_gyration_m']:.2f}m",
                    fw["flywheel_mass_kg"],
                    "kg",
                )
            )
        if getattr(s, "transient", None):
            ...
            if tr.get("air_vessel_max_gas_volume_m3") is not None:
                L.append(
                    _row("  air-vessel gas vol (max)", tr["air_vessel_max_gas_volume_m3"], "m3")
                )
    ...
    if res.staging is not None:
        ...
        L.append(
            _row(
                "Mode",
                "VFD common-speed"
                if any(s.speed_ratio not in (0.0, 1.0) for s in st.steps)
                else "fixed-speed lead/lag",
                "",
            )
        )
        ...
        L.append(
            _row(
                "Running efficiency min / mean",
                f"{sm['efficiency_min_pct']:.0f} / {sm['efficiency_mean_pct']:.0f}",
                "%",
            )
        )
        L.append(
            _row("Time outside BEP window", f"{sm['fraction_time_outside_bep'] * 100:.0f}", "%")
        )
...
def plot_performance(res: ProjectResults, path: str | Path, *, dpi: int = 130):
    ...
    if single.eff_pts is not None:
        ax2 = ax1.twinx()
        ax2.plot(
            q_lps, single.efficiency(q), color="tab:green", lw=1.4, ls=":", label="pump efficiency"
        )
