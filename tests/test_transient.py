...
def test_steady_state_is_consistent():
    pipe, pump = _case()
    r = simulate_pump_trip(pipe, pump, sump_level_m=0.0, reservoir_level_m=24.0,
                           duration_s=1.0)
...
def test_air_vessel_reduces_the_downsurge():
    pipe, pump = _case(length=2500.0)
    base = simulate_pump_trip(pipe, pump, sump_level_m=0.0, reservoir_level_m=24.0,
                              duration_s=40.0)
    prot = simulate_pump_trip(pipe, pump, sump_level_m=0.0, reservoir_level_m=24.0,
                              duration_s=40.0, air_vessel=AirVessel(gas_volume_m3=25.0))
...
def test_bigger_vessel_gives_smaller_surge():
    ...
    for v in (6.0, 12.0, 25.0):
        r = simulate_pump_trip(pipe, pump, sump_level_m=0.0, reservoir_level_m=24.0,
                               duration_s=50.0, air_vessel=AirVessel(gas_volume_m3=v))
...
def test_result_dict_shape():
    pipe, pump = _case()
    d = simulate_pump_trip(pipe, pump, sump_level_m=0.0, reservoir_level_m=24.0,
                           duration_s=20.0).as_dict()
    for k in ("max_head_m", "min_head_m", "min_gauge_pressure_head_m",
              "vapour_separation", "duration_s"):
        assert k in d
...
def test_project_moc_block(tmp_path):
    ...
    base = yaml.safe_load(
        (__import__("pathlib").Path(__file__).resolve().parents[1]
         / "examples" / "potable_water_pumping_station.yaml").read_text())
    base["water_hammer"] = {"enabled": True, "method": "moc", "moc_reaches": 16,
